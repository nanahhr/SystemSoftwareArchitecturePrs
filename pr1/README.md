...
Встановлення пакетів для розробки виконано командами:
```
sudo apt update
sudo apt upgrade -y
```
...
Перш ніж переходити до паралелізму, було розроблено спільну логіку обчислень у папці src/common. Логіка програми розділена на два модулі для імітації навантаження.

**Опис обчислювальних модулів**
* Module 1 (module1.c): Обчислює суму чисел у великому циклі ($10^9$ ітерацій);
* Module 2 (module2.c): Обчислює факторіал числа 12, використовуючи тип unsigned long long.
...
* OpenMP (Open Multi-Processing): Замість ручного створення потоків використано директиву #pragma omp parallel sections. Це дозволяє компілятору самостійно розподілити завдання між ядрами;

* POSIX AIO (aio.h): Замість звичайного виводу, який блокує роботу програми до завершення запису на диск, використано неблокуючу функцію aio_write. Це дозволяє програмі завершити обчислення, поки лог-файл ще записується системою.
...
```
**Результати:**  
Використання команди time показує різницю у швидкості обчислень завдяки прапорцю -O3.
